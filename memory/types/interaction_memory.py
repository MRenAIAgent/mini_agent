    ...
    async def store_interaction_turn(
        ...
        **metadata
        ...
        Args:
            ...
            turn_number: Sequential turn number in the session (auto-generated if not provided)
        ...
        """
        # Auto-generate turn_number if not provided
        if turn_number is None:
            # Get current session history to determine next turn number
            history = await self.get_session_history(session_id, limit=1000)
            turn_number = len(history) + 1

        interaction_metadata = {
            'memory_type': 'interaction',  # Explicitly set to avoid MRO issues
