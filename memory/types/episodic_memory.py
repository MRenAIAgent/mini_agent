...
class EpisodicMemoryManager(CoreMemoryManager):
    ...
    async def store_learning_event(
        ...
        **metadata
        ...
        """
        episodic_metadata = {
            'memory_type': 'episodic',  # Explicitly set to avoid MRO issues
    ...
    async def get_learning_episodes(
        ...
        # Use a generic query - empty string or common word
        episodes = await self.search_memory(
            query="",  # Empty query to match all, filtering is done by filters
            limit=limit * 2,
            filters=filters
        )
        ...
        if time_range:
            ...
            for episode in episodes:
                timestamp_str = episode.metadata.get('timestamp', '')
                if timestamp_str:
                    try:
                        episode_time = datetime.fromisoformat(timestamp_str)
                        if start_time <= episode_time <= end_time:
                            filtered_episodes.append(episode)
                    except (ValueError, TypeError):
                        # Skip episodes with invalid timestamps
                        continue
