    ...
    async def store_memory(
        ...
        **metadata
        ...
        try:
            ...
            if session_id:
                context = await self.context_manager.get_context(session_id, user_id=user_id)
                entry.metadata.update({
                    'session_id': session_id,
                    'context_name': context.context_name
                })
                # Only override user_id if not already set
                if 'user_id' not in entry.metadata and context.user_id:
                    entry.metadata['user_id'] = context.user_id
